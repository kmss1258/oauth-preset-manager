...
- 넓은 표의 **각 셀은 최대 두 줄**입니다. 활성 계정 정보·프리셋·캐시·오류 모두 포함하며, 빈 구분 행을 추가하지 않습니다. 다른 상세가 없는 활성 계정은 화면이 넓어도 계정 ID와 `(Current Active)`를 두 줄로 나누며, Go의 월간 상세 줄은 유지합니다. 최신 프리셋 이름 최대 2개는 계정 상세 한 줄에 묶고 넘치는 글자는 줄여 표시합니다. 추가 쿼터는 짧은 막대나 숫자 표기로 퍼센트·리셋 시간을 가능한 한 줄에 유지하며, 수집 데이터 자체는 바꾸지 않습니다.
- 실사용 API에서 확인된 OpenAI Pro/Pro Lite는 **남은 구간 `█`는 진한 무지개, 사용한 구간 `░`는 옅은 무지개**로 표시합니다. 0%에서도 옅은 무지개를 유지하며 숫자와 다른 채움 문자로 소진 상태를 구분합니다. 비컬러 출력은 기존 문자 막대를 유지합니다.
...
`라벨 [구간] 막대 퍼센트/용량 [리셋 시간]` 순서이며, 한 칸 공백만 쓰고 불릿은 넣지 않습니다. Herdr 0.8.2의 고정 토큰 구분자 `·`를 없애기 위해 한 행을 한 표시 토큰으로 묶어 **경고색은 행 전체에 적용**합니다. 긴 디스크 경로나 큰 용량은 사이드바 폭을 넓혀야 할 수 있습니다.

**표시 설정:** `opm` → **Herdr 사이드바 설정**, 또는 **`opm settings`**. 인증이나 프리셋 없이도 접근할 수 있습니다. CX·CC·Go·Disk·RAM·GPU VRAM·색상 경고를 각각 켜고 끄며, 디스크 경로 추가/제거, GPU 전체 자동 감지/개별 선택(UUID 저장), GPU 갱신 2/5/10초를 설정합니다. 저장하면 **사용자 전역**으로 실행 중인 모든 `opm q`에 수 초 내 반영하고, 취소하면 바뀌지 않습니다. 인증 설정과 별개인 `~/.config/oauth-preset-manager/sidebar.json`에 저장합니다. 잘못된 파일은 덮어쓰지 않으며 실행 중에는 마지막 유효 설정을 유지합니다.

- **Go는 5시간·월간 잔여량을 한 줄로 표시합니다.** `5h`는 5시간, `M`은 월간이며 각각 2칸 막대와 잔여 %입니다. 기존 표의 조회 결과를 60초/수동 `r` 갱신 때 공유하므로 추가 API 요청이나 인증 읽기가 없습니다. 조회 전 `Go …`, 데이터 없음 `N/A`, 조회 실패 `Go error`이며, 한 구간만 없으면 해당 구간을 `N/A`로 표시합니다. 유효한 구간 중 가장 적은 잔여율로 경고색을 정하고 `opm settings`에서 Go만 끌 수도 있습니다.
- **Disk·RAM·VRAM은 사용량/전체 용량과 찬 비율 막대**입니다. `G`는 GiB입니다. Disk는 기본 `/`, 같은 파일시스템의 경로는 한 행으로 합칩니다. Disk 사용량은 파일시스템 전체 블록에서 현재 사용자가 쓸 수 있는 `Available` 블록(`bavail`)을 뺀 값으로 계산하므로 실제 할당 가능한 잔여 용량과 일치합니다. 기존 쿼터 표 상단의 가용 공간 표시는 유지합니다.
- **경고색:** 시스템 자원 사용률 80% 이상 노랑, 90% 이상 빨강. CX/CC/Go는 잔여 20% 이하 노랑, 10% 이하 빨강. 행 전체의 색이 바뀌며, 경고를 끄면 정상색을 유지합니다.
...
*기존 쿼터 전용 예시 캡처입니다. 현재는 위 시스템 자원 행과 막대 우선 배치도 지원합니다.*

- **정상 상태에서 CX는 초록, CC는 주황**입니다. 저장된 프리셋 전체가 아니라 **활성 네이티브 Codex·Claude Code의 파일 기반 계정**입니다. %는 남은 쿼터, 시간은 리셋까지 남은 시간입니다. Codex는 5시간 구간을 우선 사용하고 없으면 실제 primary 구간을 사용합니다. 주간 전용은 `7d`, 불명확한 구간은 `quota`로 구분합니다.
...
`OPENCODE_GO_API_KEY`가 우선이고, 없으면 OPM이 선택한 활성 OpenCode auth 파일의 `opencode-go: { "type": "api", "key": "..." }`를 사용합니다. 저장된 프리셋 키 전체를 순회하지 않습니다. 동일한 인증 문맥의 캐시만 최대 24시간 사용하며, 실패한 행에는 `*`를 표시하고 원래 성공/리셋 시각을 보존합니다. 만료된 토큰은 전송하지 않습니다. 성공 조회가 다시 되면 표시를 즉시 제거하고, 429 대기 시간은 저장되어 `r`로 우회할 수 없습니다. 추론이나 인증 파일 변경은 하지 않습니다.

Go 조회 오류는 쿼터 표에서 최대 2줄로 제한합니다. **조회 403만으로 추론 불가를 뜻하지는 않습니다.** 조회 API는 키의 사용자/workspace에 Go 구독이 있어야 하지만, 추론은 잔액 결제나 다른 구독 경로로도 가능합니다. 반대로 추론은 별도의 `CreditsError`로 실패할 수 있습니다. OPM은 쿼터 수집을 위해 추론을 보내지 않습니다.
...
### 쿼터 캐시 정책
모든 쿼터 제공자는 먼저 실시간 조회를 시도합니다. 네트워크·HTTP·응답 파싱이 실패하면 같은 인증과 유효한 요청 문맥에 묶인 마지막 성공 정규화 스냅샷만 최대 24시간 표시합니다. 저장된 OpenAI OAuth, 네이티브 Codex/Claude, Google/Antigravity, OpenCode Go API 키·쿠키 workspace, Command Code에 적용됩니다. `*`는 오래된 캐시임을 뜻하며 실시간 조회가 성공하면 즉시 사라집니다. 캐시 파일에는 해시된 식별자와 정규화된 퍼센트·시각만 저장하고 토큰·쿠키·원문 응답·원문 오류·인증정보는 저장하지 않습니다. Disk·RAM·VRAM은 쿼터가 아닌 실시간 시스템 측정값이므로 캐시하지 않습니다.
