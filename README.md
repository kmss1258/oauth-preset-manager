...
- **Quota View**: View quota usage for OpenAI, Claude OAuth, Google (Antigravity), OpenCode Go, and Command Code.
  ...
  - Every wide-table cell is capped at **two physical lines**, including active account information, preset labels, cached status and errors. No blank spacer rows are inserted. Active accounts without other details keep the account ID and `(Current Active)` on separate lines even on wide screens; Go retains its monthly detail line. Up to two newest preset labels share the account detail line; overflowing text is clipped, without changing collected data. Extra quota details use shorter bars or text to keep percentage/reset information on one line where possible.
...
- **Disk/RAM/VRAM show used/total, with fullness bars.** `G` means GiB. Disk defaults to `/`; paths on the same filesystem share one row. Disk usage is calculated as filesystem blocks minus user-available blocks (`bavail`), so the displayed remaining capacity matches what the current user can actually allocate. The quota table's existing available-disk header is unchanged.
